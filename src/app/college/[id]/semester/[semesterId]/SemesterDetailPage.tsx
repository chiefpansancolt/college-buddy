...
export default function SemesterDetailPage({ collegeId, semesterId }: SemesterDetailPageProps) {
	...
	const handleUpdateClass = async (
		...
		setIsLoading(true);
		try {
			const success = await updateClass(collegeId, semesterId, editingClass.id, classData);
			if (success) {
				await loadData();
				setShowClassModal(false);
	...
	const handleUpdateAssignment = async (assignmentData: CreateAssignmentData) => {
	...
	const handleDeleteAssignment = async (assignment: Assignment) => {
		...
		try {
			await deleteAssignment(collegeId, semesterId, assignment.classId, assignment.id);
			await loadData();
			successToast({
	...
	const formatSchedule = (classItem: Class) => {
		...
		return classItem.schedule
			.map((slot) => {
				const day =
					DAYS_OF_WEEK.find((d) => d.value === slot.dayOfWeek)?.label || "Unknown";
				const location =
		...
		return (
			<div className="flex min-h-screen items-center justify-center">
				<Card className="w-full max-w-md p-6 text-center">
					...
					<Button onClick={() => router.push(`/college/${collegeId}`)} color="blue">
								...
								<Card className="py-12 text-center">
									<div className="flex flex-col items-center">
										...
										<p className="mb-6 max-w-md text-gray-600 dark:text-gray-400">
											Start adding your classes for this semester. You can
											also bulk add assignments when creating a class.
										</p>
										<Button
											onClick={() => setShowClassModal(true)}
											className="bg-blue-600 hover:bg-blue-700"
											...
											<div className="p-6">
												...
												<div className="mt-4 flex space-x-2">
													<Button
														...
														className="flex-1"
														onClick={() =>
															openAssignmentModal(classItem.id)
														}
													>
														<HiPlus className="mr-2 h-4 w-4" />
														Add Assignment
